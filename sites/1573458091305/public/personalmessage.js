var personalMessage = {
  ...
  init: function () {
    ...
    if($("#birthDay").val()!='' && $("#malayCitizens").prop("checked", true)){
      $("#idCard").blur(function(){
         var idVal = ($("#idCard").val()).substring(0,6);
         console.log(idVal);
         var birthDayVal = $("#birthDay").val();
         var birVal =  (birthDayVal.replace(/-/g,'')).substring(2,8);
         console.log(birVal);
         var sexVal = ($("#idCard").val()).substring(str.length-1);
         var sexCheck  = $(".sex").text("");
         console.log(sexCheck);
         if(sexVal%2 == 0){
          // 偶数   女
           if(sexCheck =="男"){
             alert("请输入正确的身份证号")
  ...
  getMessage: function () {
    ...
    $("#changetonameagain").html("你好<br>" + decodeURI(data.nickname));
        ...
        if(data.nationality == "0"){
          $("#malayCitizens").prop("checked", true);
        }else{
          $("#NonMalayCitizens").prop("checked", true);
        }
  ...
  if($("#idCard").val() ==''){
    ...
    $.ajax({
      ...
      url: "http://172.18.168.193:53352/sso-app/api/modify",
      type: "post",
      dataType: "json",
      data: {
        ...
        "email": $("#email").val(),
        "phone":  $('select[name=seletTxt]').val() + "-" + $("#telNum").val(),
        "name": $("#name").val(),
