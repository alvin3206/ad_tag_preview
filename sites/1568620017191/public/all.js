	...
	setTimeout(function(){
		...
		$(window).on("scroll resize", function(){
			...
			oScrollTimer = setTimeout(function(){
				...
				var $Sticky = $(".container .r1 .c2, .container .r2 .c3").eq(0);
				var iStickyPosL = $Sticky.position().left; //父容器的定位
				var iPoint_1 = $Sticky.offset().top + $Sticky.height() - $(window).height() + 300;
				$Sticky.height($Sticky.first(".inner").height()); //父子容器等高
				if (iWinScrollT > iPoint_1){
					$Sticky.find('.inner').eq(0).css({ "position": "fixed", "left": iStickyPosL, "bottom": "0px", "z-index": "50" });
				}else{
					$Sticky.find('.inner').eq(0).css({ "position": "static", "left": "auto", bottom: "auto" });
				}
			}, 100);
		});
	}, 3000);
...
$(function(){
	...
	//拷貝連結
	$("#et_sticky_pc .link").on('click', function(){
...
$(function(){
	...
	if(document.title.indexOf("404錯誤") < 0) return;
...
//20190816 Her導購模組選單hover換圖
$(function(){
	var flag = 0;  
	  $("a.her , a.her ~ .sub").hover(function(){
		if(flag == 0) {
		  $("a.her img").attr("src","https://cdn2.ettoday.net/images/4310/4310109.jpg");
		  flag = 1;
		}
		else if(flag == 1) {
		  $("a.her img").attr("src","http://cdn2.ettoday.net/images/4302/4302183.jpg");
		  flag = 0;
		}
	});	  
});
